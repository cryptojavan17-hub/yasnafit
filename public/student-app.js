(() => {
        ...
        <div class="student-modal-body">
          ...
          <div class="student-modal-video">
            ...
            <div class="mv-player ${videoSrc ? '' : 'no-video'}" style="position:relative;flex:1;min-height:185px;max-height:225px;border-radius:8px;overflow:hidden;background:var(--surface-inset);">
              ${videoSrc ? `<video controls playsinline preload="metadata" src="${esc(videoSrc)}" data-fallback-class="no-video" style="width:100%;height:100%;min-height:185px;max-height:225px;object-fit:contain;display:block;"></video>` : ''}
  ...
  async function renderProfile(){
    ...
    telegramForm.onsubmit=async event=>{event.preventDefault();const button=telegramForm.querySelector('button');button.disabled=true;const value=String(telegramForm.telegram_id.value).replace(/^@+/,'').trim();try{await api('/api/student/profile',{method:'PUT',body:jsonBody({telegram_id:value?`@${value}`:''})});toast(value?'شناسهٔ تلگرام ذخیره شد.':'شناسهٔ تلگرام حذف شد.');}catch(error){toast(error.message,'error');}finally{button.disabled=false;}};
  }
