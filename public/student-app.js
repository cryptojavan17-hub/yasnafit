(() => {
  ...
  async function renderAssessment(){
    ...
    const structured=details?`<div class="student-card" style="margin-top:13px"><h2>جزئیات پرونده</h2><div class="student-assessment-data"><div><span>نوع ارزیابی</span><b>${esc(assessment.assessment_type)}</b></div><div><span>اهداف</span><b>${esc((details.goals||[]).join('، ')||'—')}</b></div><div><span>فعالیت روزانه</span><b>${esc(details.sports?.average_daily_activity||'—')}</b></div><div><span>نوع رژیم</span><b>${esc(details.nutrition?.diet_type||'—')}</b></div><div><span>سوابق پزشکی</span><b>${details.medical?'ثبت شده':'—'}</b></div><div><span>عادات شخصی</span><b>${details.habits?'ثبت شده':'—'}</b></div></div></div>`:'';
  ...
  async function start(){
    ...
    const pages={'/student/onboarding':renderOnboarding,'/document/edit-document':renderOnboarding,'/student/dashboard':renderDashboard,'/student/program':renderProgram,'/student/assessment':renderAssessment,'/student/history':renderHistory,'/student/profile':renderProfile};
