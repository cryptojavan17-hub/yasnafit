(() => {
  ...
  async function renderOnboarding(){
    ...
    function draw(){showStepError('');document.querySelectorAll('.onboarding-step').forEach((step,index)=>step.classList.toggle('active',index===state.step));document.querySelector('#progressLabel').textContent=`مرحله ${state.step+1} از ${steps.length} — ${steps[state.step]}`;document.querySelector('#progressValue').style.width=`${(state.step+1)/steps.length*100}%`;document.querySelector('#prevStep').style.visibility=state.step===0?'hidden':'visible';document.querySelector('#nextStep').textContent=state.step===steps.length-1?'ارسال اطلاعات برای مربی':'مرحله بعد';if(state.step===4)uploadCards();if(state.step===5)review();}
