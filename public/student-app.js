(() => {
  ...
  const photoLabels={front:'جلو',back:'پشت',side:'بغل'};
  ...
  function photosHtml(photos=[]){
    if(!photos.length)return '<div class="student-empty"><span>▧</span><p>عکسی برای این ارزیابی ثبت نشده است.</p></div>';
  ...
  async function renderOnboarding(){
    ...
    function uploadCards(){
      document.querySelector('#uploadGrid').innerHTML=['front','back','side'].map(type=>{const photo=state.photos[type];return `<article class="upload-card ${photo?'has-photo':''}" data-upload-card="${type}">${photo?`<img src="/api/student-photos/${photo.id}" alt="عکس ${photoLabels[type]}"><div class="upload-card-overlay"><strong>${photoLabels[type]}</strong><div><button type="button" class="secondary" data-pick-photo="${type}">تعویض</button><button type="button" class="danger" data-remove-photo="${type}">حذف</button></div></div>`:`<strong>${photoLabels[type].toUpperCase()}</strong><p>+ افزودن عکس ${photoLabels[type]}</p><button type="button" class="secondary" data-pick-photo="${type}">انتخاب عکس</button>`}<input hidden type="file" accept="image/jpeg,image/png,image/webp" data-photo-input="${type}"></article>`;}).join('');
    ...
    async function saveStep(){
      ...
      if(state.step===1){const height=number('#obHeight'),weight=number('#obWeight');if(!height||!weight)throw new Error('قد و وزن الزامی است.');await api('/api/student/profile',{method:'PUT',body:jsonBody({height,weight})});const result=await api('/api/student/assessment',{method:'POST',body:jsonBody({height,weight,waist:number('#obWaist'),chest:number('#obChest'),hips:number('#obHips'),body_fat:number('#obFat')})});state.assessment=result.assessment;}
      ...
      if(state.step===4){const missing=['front','back','side'].filter(type=>!state.photos[type]);if(missing.length)throw new Error('عکس‌های جلو، پشت و بغل الزامی هستند.');}
    }
    function review(){document.querySelector('#reviewGrid').innerHTML=`<div class="student-review-item"><span>نام</span><b>${esc(value('#obName'))}</b></div><div class="student-review-item"><span>موبایل</span><b>${esc(value('#obMobile')||'—')}</b></div><div class="student-review-item"><span>قد و وزن</span><b>${esc(value('#obHeight'))} cm • ${esc(value('#obWeight'))} kg</b></div><div class="student-review-item"><span>هدف</span><b>${esc(value('#obGoal'))}</b></div><div class="student-review-item"><span>محدودیت‌ها</span><b>${esc(value('#obLimitations')||'—')}</b></div><div class="student-review-item"><span>عکس‌ها</span><b>جلو ✓ • پشت ✓ • بغل ✓</b></div>`;}
    function draw(){document.querySelectorAll('.onboarding-step').forEach((step,index)=>step.classList.toggle('active',index===state.step));document.querySelector('#progressLabel').textContent=`مرحله ${state.step+1} از ${steps.length} — ${steps[state.step]}`;document.querySelector('#progressValue').style.width=`${(state.step+1)/steps.length*100}%`;document.querySelector('#prevStep').style.visibility=state.step===0?'hidden':'visible';document.querySelector('#nextStep').textContent=state.step===steps.length-1?'ارسال اطلاعات برای مربی':'مرحله بعد';if(state.step===4)uploadCards();if(state.step===5)review();}
    document.querySelector('#prevStep').onclick=()=>{if(state.step>0){state.step--;draw();}};
    document.querySelector('#nextStep').onclick=async event=>{event.currentTarget.disabled=true;try{if(state.step===steps.length-1){if(!document.querySelector('#confirmAssessment').checked)throw new Error('صحت اطلاعات را تأیید کنید.');event.currentTarget.textContent='در حال ارسال اطلاعات...';await api('/api/student/assessment/submit',{method:'POST'});return renderSuccess();}await saveStep();state.step++;draw();}catch(error){toast(error.message,'error');}finally{event.currentTarget.disabled=false;}};
    draw();
  }
