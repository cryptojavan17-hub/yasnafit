(() => {
  ...
  async function renderProfile(){
    ...
    telegramForm.onsubmit=async event=>{event.preventDefault();const button=telegramForm.querySelector('button');button.disabled=true;const value=String(telegramForm.telegram_id.value).replace(/^@+/,'').trim();try{await api('/api/student/profile',{method:'PUT',body:jsonBody({telegram_id:value?`@${value}`:''})});toast(value?'اتصال به ربات تلگرام به‌روزرسانی شد.':'اطلاعات ربات تلگرام حذف شد.');}catch(error){toast(error.message,'error');}finally{button.disabled=false;}};
  }
