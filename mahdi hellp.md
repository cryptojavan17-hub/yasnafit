...
> آخرین به‌روزرسانی: **2026-09-20** (توسط Agent جلسهٔ `arena/01a0b993-yasnafit` — **Task 21 (شماره‌گذاری docs/project-tracking): لندینگ = تصویر کامل طرح مرجع `landing2.png` (دستور مالک: «همین تصویر مرجع رو کامل استفاده کن؛ بعدا جای دکمه ها رو مشخص میکنیم») ⇒ دکمه‌ها/المان‌های تعاملی بعدا (T-19)**. قبل‌تر: Task 17/18/19/20 (همه committed + push شده). جلسهٔ قبل: `arena/01a0817f-yasnafit` — Task 25/26).
...
* **مستندات پیگیری:** `docs/project-tracking/` → `PROJECT-CONTEXT.md` (کسب‌وکار + قواعد)، `CHANGELOG.md` (توسعه/تسک‌ها — Task 1…21)، `KNOWN-ISSUES.md` (KI-001…KI-018)، `TODO.md` (T-01…T-19)، `TECHNICAL-DECISIONS.md` (TD-1…TD-24)، `archive/`.
...
| آخرین کامیت | Task 17–21 همه committed (آخرین = کامیت Task 21) — مبنا `6301712` |
| working tree | **تمیز** — همهٔ Task 17–21 (سایت عمومی + wireframe + بازسازی + launcher + تصویرها + تصویر کامل لندینگ) committed روی `arena/01a0b993-yasnafit` و push شده (2026-09-20) |
...
| اقدامات باز | (۱) مالک: apply/merge شاخهٔ جلسه (روش معمول خودش) (۲) **T-19: جای‌گذاری دکمه‌ها روی تصویر کامل لندینگ — مالک جای‌ها را مشخص می‌کند** (۳) T-17 (auto-fetch) فقط با تأیید مالک (۴) T-18 باقی‌مانده: فقط تصویر OG (اختیاری) (۵) تصمیم «غیرفعال‌کردن گوگل‌اتنتیکاتور» از جلسهٔ قبل هنوز بی‌پاسخ است |
...
* (به‌روز ۲۰۲۶-۰۹-۲۰ — Task 21) **T-17: موتور auto-fetch مقالات** (P2 — فقط با تأیید مالک) • **T-18: تصاویر نهایی برند سایت عمومی** (PARTIAL — هیرو/درباره/CTA + ۴ کارت مجله DONE؛ فقط تصویر OG باقی است؛ KI-018 بسته شد) • **T-19: جای‌گذاری دکمه‌ها روی تصویر کامل لندینگ** (P1 — مالک جای‌ها را مشخص می‌کند، Task 21).
...
4. **T-19:** جای‌گذاری دکمه‌ها/المان‌های تعاملی روی تصویر کامل لندینگ — پس از مشخص‌شدن جای‌ها توسط مالک (دستور Task 21).
...
2. کار بعدی فقط با درخواست مالک (کارهای باز فعلی: T-19 جای‌گذاری دکمه‌ها روی تصویر لندینگ + T-17/P2 + T-18-OG اختیاری — فقط با تأیید مالک).
