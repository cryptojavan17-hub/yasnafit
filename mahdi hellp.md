...
| آخرین کامیت | **`main` = `22bfab6`** (merge PR #6 در 2026-09-08 **16:36:57Z** با تأیید صریح مالک) شامل **Task 26** (`5e65c5b`) + **Task 25** (`ca53577`) + `63a17cb` + `583af1b`؛ شاخهٔ جلسهٔ `arena/01a0817f-yasnafit` در `5e65c5b` است و کامیت‌های docs بعد از merge روی همین شاخه می‌نشینند |
...
| local vs origin | شاخهٔ جلسهٔ `arena/01a0817f-yasnafit` = `5e65c5b` و push شده (local == remote)؛ `main` = **`22bfab6`** ⇒ شاخهٔ جلسه یک کامیتِ merge از main عقب است که طبیعی است (PR بعدی فقط diff جدید را نشان می‌دهد) |
| PR | **#6 MERGED** (Task 25 مستندات + Task 26 UI بانک حرکات → main؛ 2026-09-08 **16:36:57Z** با تأیید صریح مالک — merge commit `22bfab6`) • #5 MERGED (Task 24 — `607587f`) • #4 MERGED (Task 23) • #3 MERGED • #2 MERGED • #1 CLOSED |
| وضعیت merge | محتوای PR #6 روی `main` = `22bfab6` است و **auto-deploy منتشر کرد**؛ دو شاهد مستقل از بیرون: (۱) **ریست uptime** — `uptime:1329` در ~16:59Z ⇒ استارت پروسه ~**16:37Z** یعنی دقیقاً بعد از merge (۲) **خواندن کد سرو‌شده** — `GET /program-builder.css` روی production قواعد جدید (`.quickadd-submit`، `.drawer-manual-toggle`، `.drawer-setpreset`، `.drawer-preset-select` + media query موبایل ۴۶px) را دارد و قواعد حذف‌شدهٔ آناتومی (`.mv-anatomy`/`.mv-figures`/`.muscle-container`/`.base-body`/`.muscle-overlay`/`.mv-muscle-select`) در آن **نیستند**؛ `.mv-muscle-chip` (مورد نیاز `exercises.js`) عمداً باقی است و `.mv-learn` تک‌ستونه شده ⇒ **Task 26 live است** |
| اقدامات باز | (۱) **مالک: تست چشمی Task 26 روی production** با `Ctrl+Shift+R` (کش JS بیست‌وچهار ساعته): درشت‌دیدن «＋ افزودن حرکت دستی» و «ثبت حرکت» در موبایل • نوار «ست‌های حرکت» زیر پنل • انتخاب «۴ * ۱۰» ⇒ حرکت اضافه‌شده از بانک همان ۴ ست را داشته باشد • فرم ویرایش حرکت بدون «عضله هدف» ولی با «آموزش حرکت» (۲) **مالک: تصمیم «غیرفعال‌کردن گوگل‌اتنتیکاتور»** — چهار گزینه در §۱۴ بند ۹؛ **هیچ تغییری در کد احراز هویت داده نشد** (۳) مهدی در ویندوز: بلوک bat §17 (۴) پاک‌کردن `YASNAFIT_ALLOW_2FA_SKIP`/`YASNAFIT_ALLOW_REMOTE_SETUP` **فقط اگر** تصمیم بند ۲ «حفظ 2FA» باشد (۵) اختیاری با تأیید مالک: cache-busting نسخه‌دار اسکریپت‌ها (۶) اختیاری و فقط خواندنی: `KI-016` |
...
* **وضعیت زنده (۲۰۲۶-۰۹-۰۸ عصر — جلسهٔ `arena/01a0817f-yasnafit`؛ انتشار Task 25 + Task 26):** PR #6 با تأیید صریح مالک در **16:36:57Z** merge شد ⇒ `main` = `22bfab6`. راستی‌آزمایی از بیرون با دو شاهد: ریست uptime (`uptime:1329` در ~16:59Z ⇒ استارت ~16:37Z) و **خواندن `/program-builder.css` سرو‌شده روی production** که قواعد جدید Task 26 را دارد و قواعد حذف‌شدهٔ آناتومی در آن نیست (`.mv-muscle-chip` عمداً باقی است؛ `.mv-learn` تک‌ستونه). ⚠️ نکتهٔ صادقانه: **پیش از merge** هم یک ری‌استارت ~**15:30Z** دیده شد (`uptime:3340` در 16:25:40Z در برابر `uptime:418274` در 14:54:25Z) که علتش از بیرون قابل تشخیص نیست؛ چون `main` هنوز `607587f` بود، کد جدید را نیاورد — اگر تکرار شد لاگ Railway را ببینید (ری‌استارت پلتفرم / تغییر متغیر محیطی / redeploy دستی).
...
* **وضعیت نهایی:** ✅ **انجام شد، merge شد و منتشر شد** — PR #6 با تأیید صریح مالک در 2026-09-08T16:36:57Z merge شد ⇒ `main` = `22bfab6`؛ انتشار با **ریست uptime** (~16:37Z) و **خواندن CSS سرو‌شده روی production** اثبات شد. `npm test` = ۱۸/۱۸ (exit 0). قدم بعدی: تست چشمی مالک (سه مورد §۱۴) با `Ctrl+Shift+R`؛ و تصمیم بند ۹ (2FA) که هنوز در انتظار است.
...
| Local (سندباکس Agent) | **CURRENT** | شاخهٔ `arena/01a0817f-yasnafit` = `5e65c5b` (Task 26) که **روی `main` merge شد** (`22bfab6`)؛ کامیت‌های docs بعد از merge روی همین شاخه می‌نشینند |
| Git / GitHub origin | **CURRENT** | `main` = **`22bfab6`** (merge PR #6 = Task 25 + Task 26)؛ شاخهٔ جلسه push شده؛ PR بعدی از همین شاخه باز می‌شود |
| `main` | **`22bfab6` — Task 25 + Task 26 روی main** | build بعد از merge موفق شد (کد سرو‌شده روی production گواهی آن است) |
| Railway | **LIVE (0.9.1) — کد Task 26 اجرا می‌شود** (اثبات: ریست uptime ~16:37Z + CSS سرو‌شده روی production) | Auto-deploy روشن (Branch=`main`)؛ ۱۸۸۸ عکس روی Volume؛ KI-015 بسته. ⚠️ یک ری‌استارتِ دیگر هم ~**15:30Z** همان روز دیده شد (**قبل از** merge؛ علتش از بیرون قابل تشخیص نیست — احتمالاً redeploy دستی/تغییر متغیر/ری‌استارت پلتفرم) و چون `main` هنوز `607587f` بود، کد جدید را منتشر نکرد |
