...
for(const token of ['YASNAFIT_TRUST_PROXY','YASNAFIT_HOST','YASNAFIT_COOKIE_SECURE','proxy_set_header X-Forwarded-For','systemd','22.5','chmod 700','/api/backup']){
...
// the 2FA login step was removed entirely: single-step email+password login
assert.ok(!/REVEAL_AUTHENTICATOR_KEY|ALLOW_2FA_SKIP/.test(serverSource),'the obsolete 2FA env switches must be gone from the server');
assert.ok(!/REVEAL_AUTHENTICATOR_KEY|ALLOW_2FA_SKIP/.test(read('src/request-security.js')||''),'the obsolete 2FA env switches must be gone from request-security');
assert.ok(!exists('scripts/provision-coach-totp.js'),'the TOTP provisioning script must be deleted with the 2FA step');
assert.ok(!exists('public/coach-2fa.html'),'the 2FA page must be deleted');
assert.match(serverSource,/skipTotp:true/,'coach login must complete in one step (no OTP challenge)');
assert.ok(!/auth\/verify|auth\/challenge/.test(serverSource),'the OTP endpoints must be gone');
assert.match(serverSource,/url\.pathname==='\/coach\/2fa'/,'the stale 2FA URL must redirect to the login page');
assert.match(read('public/coach-login.js'),/location\.replace\(data\.next \|\| '\/coach\/dashboard'\)/,'login must land directly on the dashboard');
