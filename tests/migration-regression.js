...
try {
  ...
  assert.equal(clean.prepare('SELECT COUNT(*) AS c FROM releases').get().c,13);
  ...
  assert.equal(upgrade.prepare('SELECT COUNT(*) AS c FROM releases').get().c,13);
  ...
  assert.equal(legacy.prepare('SELECT COUNT(*) AS c FROM releases').get().c,13);
  legacy.close();
  console.log(JSON.stringify({ok:true,clean_migrations:migrations.length,upgrade:'006->018',releases:13,legacy_missing_updated_at:'repaired',student_sessions:true,integrity:'ok'}));
} finally {
