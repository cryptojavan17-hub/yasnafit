...
try {
  ...
  // Upgrade from schema 006 with duplicate current photo slots. Migration 007
  // must preserve rows, soft-delete the superseded slot, and remain idempotent.
  ...
  for(const migration of migrations.filter(m=>m.id!=='007_monthly_workflow_integrity')){
    upgrade.exec('BEGIN');
  ...
  upgrade.close();
  console.log(JSON.stringify({ok:true,clean_migrations:migrations.length,upgrade:'006->007',integrity:'ok'}));
} finally {
