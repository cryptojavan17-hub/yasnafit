...
try {
  ...
  for(const table of ['coaches','coach_students','assessment_ai_suggestions','coach_sessions','coach_otp_challenges','coach_password_resets','coach_auth_events'])assert.ok(clean.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(table),`missing ownership table ${table}`);
  for(const column of ['email','email_normalized','password_hash','auth_failed_attempts','auth_locked_until','last_login_at','role','totp_secret','totp_confirmed_at','totp_last_counter'])assert.ok(clean.prepare("SELECT 1 FROM pragma_table_info('coaches') WHERE name=?").get(column),`missing coach auth column ${column}`);
  ...
  for(const table of ['magazine_categories','magazine_articles','magazine_article_sources','success_stories'])assert.ok(clean.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(table),`missing public site table ${table}`);
  assert.equal(clean.prepare("SELECT COUNT(*) AS c FROM magazine_categories").get().c,5);
  assert.equal(clean.prepare("SELECT slug FROM magazine_categories ORDER BY sort_order,id").all().map(r=>r.slug).join(','),'bodybuilding,sports-science,nutrition,health,sports-news');
  clean.close();
