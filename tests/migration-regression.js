...
try {
  ...
  for(const column of ['email','email_normalized','password_hash','auth_failed_attempts','auth_locked_until','last_login_at','role'])assert.ok(clean.prepare("SELECT 1 FROM pragma_table_info('coaches') WHERE name=?").get(column),`missing coach auth column ${column}`);
