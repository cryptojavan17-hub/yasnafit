...
try {
  ...
  for(const table of ['coaches','coach_students','assessment_ai_suggestions','coach_sessions','coach_otp_challenges'])assert.ok(clean.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(table),`missing ownership table ${table}`);
  for(const column of ['email','email_normalized','password_hash','auth_failed_attempts','auth_locked_until','last_login_at'])assert.ok(clean.prepare("SELECT 1 FROM pragma_table_info('coaches') WHERE name=?").get(column),`missing coach auth column ${column}`);
