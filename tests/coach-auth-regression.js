...
try{
  ...
  const qr=totp.qrSvg(totp.otpauthUrl({secret:liveSecret,email:'mehdi.javan.64@gmail.com'}));
  assert.match(qr,/^<svg /);
  assert.match(qr,/<\/svg>$/);
  ...
  assert.equal(first.setup_email,'mehdi.javan.64@gmail.com');
  ...
  const blocked=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  assert.equal(blocked.error,'SETUP_REQUIRED');

  assert.throws(()=>auth.setupCoach(db,{email:'other@example.com',password:'YasnafitCoach1'}),/mehdi\.javan\.64@gmail\.com/);
  assert.throws(()=>auth.validateCoachPassword('short1'),/حداقل ۸/);
  assert.throws(()=>auth.validateCoachPassword('abcdefgh'),/حرف و عدد/);
  const created=auth.setupCoach(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',displayName:'مربی'});
  assert.equal(created.email,'mehdi.javan.64@gmail.com');
  ...
  assert.throws(()=>auth.setupCoach(db,{email:'mehdi.javan.64@gmail.com',password:'AnotherPass9'}),/قبلاً ساخته شده/);
  ...
  assert.equal(auth.normalizeEmail('Crypto.Javan17@Gmail.com'),'crypto.javan17@gmail.com'); // normalize فقط lowercase می‌کند — ورودی دلخواه مجاز است
  ...
  const unconfirmedLogin=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  ...
  const wrong=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'WrongPass1',dataDir});
  assert.equal(wrong.error,'INVALID_CREDENTIALS');
  const started=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  assert.ok(started.challenge_id);
  assert.equal(started.email,'mehdi.javan.64@gmail.com');
  ...
  assert.equal(resolved.coach.email,'mehdi.javan.64@gmail.com');
  ...
  const fileLogin=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  ...
  const otpLockLogin=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  ...
  const afterOtpLock=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  ...
  const expireLogin=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  ...
  const noFlag=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  ...
  const skipLogin=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir,skipTotp:true});
  ...
  const skipBadPassword=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'WrongPass1',dataDir,skipTotp:true});
  ...
  const sessionA=auth.verifyOtp(db,{challengeId:(await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir})).challenge_id,code:loginCode(db,secret)});
  const sessionB=auth.verifyOtp(db,{challengeId:(await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir})).challenge_id,code:loginCode(db,secret)});
  ...
  const liveLogin=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  ...
  const afterChange=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1',dataDir});
  assert.equal(afterChange.error,'INVALID_CREDENTIALS');

  auth.setMailer(async payload=>delivered.push(payload));
  delivered.length=0;
  const forgot=await auth.requestPasswordReset(db,{email:'mehdi.javan.64@gmail.com',dataDir,req:{headers:{host:'localhost:3020'},socket:{}}});
  ...
  const afterReset=await auth.startLogin(db,{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach3',dataDir});
  assert.ok(afterReset.challenge_id);
  assert.equal(afterReset.code,undefined);

  auth.setMailer(null);
  const fileForgot=await auth.requestPasswordReset(db,{email:'mehdi.javan.64@gmail.com',dataDir,req:{headers:{host:'localhost:3020'},socket:{}}});
  ...
  // auth flow is checked against a stub DOM: markup -> listener -> fetch -> navigation.
  {
    ...
    elements.get('coachEmail').value='mehdi.javan.64@gmail.com';
    ...
    assert.deepEqual(JSON.parse(post[3]),{email:'mehdi.javan.64@gmail.com',password:'YasnafitCoach1'});
