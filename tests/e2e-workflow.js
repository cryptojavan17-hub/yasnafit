...
(async()=>{
  ...
  await ok('/api/student/profile',{method:'PUT',cookie:sessionA.cookie,body:{full_name:`Student A ${suffix}`,mobile:mobileA,telegram_id:'@yasnafit_test',instagram_id:'@yasnafit.test',height:175,weight:78,goal:'فیتنس',training_experience:'متوسط',preferred_location:'gym',limitations:'none',injuries:'none'}});const meA=await ok('/api/student/me',{cookie:sessionA.cookie});assert.equal(meA.student.telegram_id,'@yasnafit_test');assert.equal(meA.student.instagram_id,'@yasnafit.test');assert.equal(meA.student.case_number,a.case_number);const profileShell=await request('/student/profile',{cookie:sessionA.cookie});assert.equal(profileShell.response.status,200);const studentAppSrc=fs.readFileSync(path.join(__dirname,'..','public','student-app.js'),'utf8');assert.match(studentAppSrc,/id="telegramForm"/,'student profile: telegram bot form (client-rendered)');assert.match(studentAppSrc,/name="telegram_id"/,'student profile: telegram_id input wired to /api/student/profile');assert.match(studentAppSrc,/student\.telegram_id/,'student profile: telegram handle prefilled from the account');
