...
## 🖼 تعویض تصویر هیروی صفحه ورود

صفحه ورود (طراحی لوکس دارک) از تصویر `public/images/auth-hero.jpg` به‌عنوان هیرو استفاده می‌کند.

- برای قرار دادن عکس خودتان: فایل عکس را با همین نام (`auth-hero.jpg`) در `public/images/` جایگزین کنید و برنامه را ری‌استارت کنید.
- عکس عمودی/پرتره با پس‌زمینه تاریک بهترین نتیجه را می‌دهد (برش خودکار با `object-fit: cover`).
- اگر فایل موجود نباشد، صفحه به‌صورت خودکار با گرادیان لوکس نمایش داده می‌شود و خراب نمی‌شود.
