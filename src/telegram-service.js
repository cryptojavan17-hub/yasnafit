...
// ── ورود مهمان (دکمهٔ لندینگ: https://t.me/<bot>?start=landing) ──
// Owner spec 2026-09-21 (revised the same day): a *respectful* welcome first, then guidance.
// Nothing is stored for guests; account linking still needs a one-time code issued to a
// signed-in student/coach (the /start <code> flow below).
const DEFAULT_PUBLIC_URL = 'https://yasnafit.ir';
const GUEST_ENTRY_PAYLOADS = new Set(['landing', 'site', 'web', 'home', 'start']);
function siteUrl(pathname){ return portalUrl(pathname) || `${DEFAULT_PUBLIC_URL}${pathname}`; }
function guestButtons(){
  return [
    [{ text: '📝 ثبت‌نام در سایت', url: siteUrl('/student/register') }],
    [{ text: '🔑 ورود به پنل شخصی', url: siteUrl('/student/login') }],
  ];
}
function guestWelcomeText(){
  const register = escapeHtml(siteUrl('/student/register'));
  const login = escapeHtml(siteUrl('/student/login'));
  return [
    'سلام و درود 👋',
    ...
    '۳) سپس از پنل شخصی → «پروفایل من» → «اتصال تلگرام» با یک کلیک حساب خود را به همین ربات متصل کنید تا اعلان آماده‌شدن برنامه و پیام‌های مربی را در تلگرام دریافت کنید.',
    '',
...
function guestGuideText(){
  const register = escapeHtml(siteUrl('/student/register'));
  const login = escapeHtml(siteUrl('/student/login'));
  return [
    '📖 <b>راهنمای ربات یسنا فیت</b>',
    '',
    'هنوز حسابی به این گفتگو متصل نشده است؛ این ربات فعلاً شما را تا ثبت‌نام و اتصال حساب راهنمایی می‌کند.',
    '',
    'دستورها:',
    ...
    '🌐 ثبت‌نام در سایت:',
    `<a href="${register}">${register}</a>`,
    '🔑 ورود به پنل شخصی (برای اتصال حساب: «پروفایل من» → «اتصال تلگرام»):',
    `<a href="${login}">${login}</a>`,
    '',
    'مربی گرامی: پنل مدیریت → سیستم → تنظیمات تلگرام → «اتصال تلگرام مربی».',
  ].join('\n');
}
...
async function handleMessage(db, message){
  ...
  // /start landing (button on the public site) is a guest entry, never a connection code.
  ...
  if(command === '/start' && startPayload && !isEntryPayload){
    ...
    try{
      ...
      await sendMessage(db, chat.chat_id,
        `⚠️ ${reason}\n\nراهنمایی:\n• شاگردان: پنل شخصی یسنا فیت → «پروفایل من» → «اتصال تلگرام» → دکمهٔ اتصال خودکار (کد جدید همان‌جا ساخته می‌شود).\n• مربی: پنل مدیریت → سیستم → تنظیمات تلگرام → «اتصال تلگرام مربی».\n\nهنوز ثبت‌نام نکرده‌اید؟ /start را بفرستید تا راهنمای شروع را دریافت کنید.`,
        guestButtons());
  ...
  if(command === '/start' || command === '/menu'){
    ...
    await sendMessage(db, chat.chat_id, guestWelcomeText(), guestButtons());
    return { handled: true, guest: true, from_landing: isEntryPayload };
  }
  ...
  if(!account){
    await sendMessage(db, chat.chat_id, guestGuideText(), guestButtons());
    return { handled: true, no_account: true, guest_guide: true };
  }
