...
const DIET=['none','no_restriction','vegetarian','vegan','celiac','lactose_intolerance','gout','low_carb','ketogenic','fasting','professional','competition','iranian'];
...
function text(value,name,max=50000,required=false){const output=String(value??'').trim();if(required&&!output)throw new Error(`${name} الزامی است`);if(output.length>max)return output.slice(0,max);return output;}
