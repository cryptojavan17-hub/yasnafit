...
function saveSection(db,assessmentId,studentId,section,data={}){
  ...
  try{
    if(section==='general'){
      ...
      const values={height:number(data.height,'قد',100,250,true),weight:number(data.weight,'وزن',20,300,true),around_the_arm:number(data.around_the_arm,'دور بازو',10,100),around_the_chest:number(data.around_the_chest,'دور سینه',30,250),around_the_belly:number(data.around_the_belly,'دور شکم',30,250),around_the_belly_from_the_navel:number(data.around_the_belly_from_the_navel,'دور ناف',30,250),around_the_hips:number(data.around_the_hips,'دور باسن',30,250),around_the_leg:number(data.around_the_leg,'دور ساق',10,120),around_the_thigh:number(data.around_the_thigh,'دور ران',20,150),around_the_wrist:number(data.around_the_wrist,'دور مچ',5,50)};upsert(db,'assessment_measurements',assessmentId,values);db.prepare('UPDATE body_assessments SET height=?,weight=?,chest=?,waist=?,hips=?,draft_saved_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP,version=version+1 WHERE id=?').run(values.height,values.weight,values.around_the_chest,values.around_the_belly_from_the_navel,values.around_the_hips,assessmentId);
