...
- **WHAT:**
  1. **معماری سه‌لایه:** رویداد کسب‌وکار → `notificationService.emit` (گیت ترجیحات + dedup + دفتر تحویل) → آداپتور تلگرام (پایان‌پذیر؛ ایمیل/پوش آینده فقط نقطهٔ توسعه در کاتالوگ TYPES هستند و رفتار ساختگی ندارند). تلگرام هرگز منبع حقیقت نیست و هیچ منطق تلگرامی داخل کد کسب‌وکار نیست.
  2. **مایگریشن 031** (`031_telegram_integration`): جداول `telegram_accounts`، `telegram_link_tokens`، `notification_preferences`، `notification_deliveries` + ایندکس‌ها و یونیک‌های جزئی (یک حساب فعال برای هر چت / هر شاگرد، dedup_key یکتا). مایگریشن‌های قبلی دست نخوردند.
  3. **اتصال امن:** شاگردِ واردشده از پورتال کد یک‌بارمصرف ۱۵ دقیقه‌ای می‌گیرد (ذخیره به‌صورت sha256 + نمایش یک‌بارهٔ کد خام) و با `/start <کد>` در ربات متصل می‌شود؛ مصرف موفق توکن را باطل می‌کند؛ اتصال مجدد، اتصال قبلی هر دو طرف را آزاد می‌کند؛ هر چت فقط به یک شاگرد قابل اتصال است. هویت فقط از `from.id` تلگرام (نه متن) — ورود با «شناسه/موبایل شاگرد دیگر» از اساس ممکن نیست.
  4. **رویدادهای واقعی وصل‌شده (فقط موجودها):** PROGRAM_ASSIGNED (فعال‌سازی برنامه)، PROGRAM_UPDATED (ویرایش)، NUTRITION_PLAN_READY (ایجاد/به‌روزرسانی رژیم)، SUPPLEMENT_PLAN_READY (ایجاد/به‌روزرسانی مکمل)، COACH_MESSAGE (پیام جدید مربی)، TELEGRAM_CONNECTED/DISCONNECTED، PROGRAM_ENDING_REMINDER (در کاتالوگ؛ شِدول‌بند فعلاً وصل نیست). هیچ رویداد خیالی (پرداخت/نوبت‌دهی) وجود ندارد.
  5. **موتور تحویل:** dedup با کلید پایدار (رویداد تکراری = یک پیام)، ۳ تلاش با تأخیر ۱/۵/۱۵ دقیقه، تایم‌اوت ۱۵ ثانیه، خطای دائم ۴۰۳ (بلاک ربات) → وضعیت حساب `blocked`، ۴۰۰ «chat not found» → `invalid`، تحویل `cancelled`؛ خطای شبکه/۵xx → `retrying`. خطای تلگرام هرگز مسیر اصلی Yasnafit را نمی‌شکند (همه emitها try/catch).
  6. **ربات فارسی RTL:** `/start` `/help` `/status` `/program` `/nutrition` `/notifications` `/settings` `/unlink` + کیبورد درون‌خطی اعلان‌ها (`pref:<key>`) + دکمه «مشاهده برنامه تمرینی» با لینک پورتال (`YASNAFIT_PUBLIC_URL`). هیچ شناسه دیتابیسی در پیام‌ها نیست؛ chat_id فقط ماسک‌شده (`••••7001`) در APIها برمی‌گردد.
  7. **وب‌هوک داخل همان سرور HTTP موجود** (`POST /api/telegram/webhook`، بدون Express) با اعتبارسنجی هدر مخفی `X-Telegram-Bot-Api-Secret-Token` به‌صورت timing-safe؛ برای توسعهٔ محلی `TELEGRAM_POLLING=1` (long-polling) یا تونل. بدون پیکربندی، کل قابلیت بی‌صدا غیرفعال است (`[Telegram] غیرفعال`) و پورتال عادی کار می‌کند.
  8. **پنل‌ها:** کارت «🔔 اتصال تلگرام» در پروفایل شاگرد (اتصال با دیپ‌لینک/کد، قطع اتصال، تاگل ۵ دسته اعلان) و ردیف «تلگرام» در پروندهٔ شاگرد مربی (وضعیت اتصال/بلاک/نامعتبر + آخرین موفقیت/خطا) — هر دو با CSS توکن‌محور، خارج از inline (CSP).
- **WHY:** مالک خواستار اعلان‌های قابل‌اعتماد تلگرام برای رویدادهای واقعی برنامه/تغذیه/پیام بود؛ اولویت اعلان اول: «برنامه تمرینی جدید آماده شد».
- **TESTS:** سوئیت جدید `tests/telegram-integration-regression.js` با transport تزریقی (بدون ارسال واقعی): اتصال معتبر/نامعتبر/منقضی/استفاده‌شده، قطع و اتصال مجدد و مالکیت چت، تحویل موفق با پیام دقیق برنامه، dedup، retry تا failed، بلاک ربات، چت نامعتبر، شاگرد بدون تلگرام، ترجیح خاموش، امضای وب‌هوک، کامندهای فارسی، تاگل درون‌خطی، سیم‌کشی تریگر واقعی سرور، عملکرد بدون پیکربندی — ۱۶ گروه ✅ • `node tests/migration-regression.js` ✅ (۳۱ مایگریشن، integrity ok) • e2e زندهٔ HTTP: ورود مربی/شاگرد → link ۲۰۱ → وب‌هوک با رمز غلط ۴۰۱ → `/start` با رمز درست → status متصل با chat ماسک‌شده → قطع اتصال ✅ • بوت بدون پیکربندی: پورتال بالا، تلگرام غیرفعال ✅ • `npm test` کامل ✅
- **RESULT:** PASS — دو باگ حین تست گرفته شد: (۱) شادو کردن `telegramService` داخل `notification-service.emit` (TDZ) که کل emit را می‌شکست؛ (۲) `JSON.parse` دوباره روی خروجی `readBody` در مسیر وب‌هوک که هر آپدیت معتبر را ۴۰۱/۴۰۰ می‌کرد.
- **ENV:** `TELEGRAM_BOT_TOKEN`، `TELEGRAM_BOT_USERNAME`، `TELEGRAM_WEBHOOK_SECRET`، `YASNAFIT_PUBLIC_URL`، `TELEGRAM_POLLING` (همه اختیاری — در `.env.example` مستند شد؛ `.env` به Git نمی‌رود).
