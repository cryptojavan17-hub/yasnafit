...
│   ├── magazine-discovery-service.js ← (Task 27 + Correction 9) خط سامپاد مجله: منابع RSS/Atom + تیرهای کیفیت (037)، دروازهٔ تازگی ≤۳۰/۹۰روز (+always-green از AI)، آماده‌سازی ویرایشی AI **الزامی** (فارسی: عنوان/خلاصه/چرا مهم/نکات؛ بدون AI = بدون پیش‌نویس)، منبع نهایی = انتشارکنندۀ اصلی (نه گوگل‌نیوز)، تصویر همان مقاله/og/«تصویر موجود نیست»+انتخاب مربی، تکراری‌زدایی ۴لایه (url_hash/redirect + عنوان + URL + داستان شبه‌تکراری)، صف بررسی، backfill DRAFT، زمان‌بند، audit
