...
**Task 29 — همان روز (پرامپت دوم مالک، `arena/01a0c4f9-yasnafit`):** ربات `@yasnafitbot` حالا سمت سرور پاسخ‌گو است: `src/telegram-bot-service.js` (بدون وابستگی) به `/start` و `/start landing` پیام خوش‌آمد فارسی مالک + لینک `https://yasnafit.ir/student/register` (لینک + دکمهٔ inline) و به `/help`/پیام‌های دیگر راهنمای کوتاه می‌دهد. دریافت update: webhook `POST /api/telegram/webhook` (پروداکشن؛ محافظت با `X-Telegram-Bot-Api-Secret-Token`) یا long polling (توسعه). توکن فقط از `TELEGRAM_BOT_TOKEN` (هرگز لاگ نمی‌شود). **هیچ ذخیره‌ای نیست** (نه chat_id، نه username، نه اتصال حساب) — طبق دامنهٔ صریح مالک. وضعیت: کد `IMPLEMENTED` و با Bot API جعلی `VERIFIED` (۱۹ گروه)؛ پذیرش با ربات زنده `UNKNOWN — needs verification` تا مالک متغیرها را روی Railway ست کند (`DEPLOYMENT.md` §۱۰).
