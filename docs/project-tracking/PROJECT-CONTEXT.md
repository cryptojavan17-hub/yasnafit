...
**آخرین به‌روزرسانی:** 2026-09-20 — **Task 27 (T-17 + اصلاح ۵): نوار پیشرفت زندهٔ «بررسی مطالب جدید» (فازها: منابع → گزارش فارسی → تصویرها + نام منبع فعلی + شمارنده‌ها + گزارش خطا/تکمیل) و رفع «تصویر بارگذاری نشد» (og http→https + referrerpolicy no-referrer)؛ ۶۵ چک + npm test + e2e (schema ۳۶) — قبل‌تر: اصلاح ۴ (تصویر/سلسله‌مراتب منابع) + اصلاح ۳ (Inbox) + اصلاح ۲ + Task ۲۵ مرحلهٔ ۳ + Task ۲۶**
...
│   ├── magazine-discovery-service.js ← (Task 27) خط سامپاد مجله: منابع RSS/Atom، تکراری‌زدایی سه‌لایه، پیش‌نویس (AI/حداقلی + پرچم‌های کیفیت)، صف بررسی، زمان‌بند
...
| Editorial Admin (پنل مجله) | **IMPLEMENTED** `VERIFIED` (Task 17؛ **Task 27 — 2026-09-20: خط سامپاد + Inbox**) | `/coach/magazine` در سایدبار؛ تب‌های: مقالات / **اخبار و مطالب جدید** (Inbox ویرایشی: «آخرین بررسی» + «AI N مطلب جدید پیدا کرده است» + «🔍 بررسی مطالب جدید» + کارت‌ها: اموجی دسته/عنوان فارسی+Original/منبع/تاریخ/خلاصه/«چرا مهم است»/نکات کلیدی/تصویر یا «انتخاب تصویر»/لینک منبع اصلی + عملیات مشاهده کامل/ویرایش/✓ انتشار/رد + صفحهٔ بررسی دوستونه برای «مشاهده کامل») / نتایج شاگردان / دسته‌بندی‌ها / تنظیمات؛ **تب فنی «منابع خبری» از UI مربی حذف شد** (۱۱ منبع معتبر پیش‌فرض — PubMed/Google News برای ورزش/تغذیه/بدنسازی/ریکاوری/خواب/مکمل/ورزش زنان — فقط در بک‌اند + API برای تشخیص فنی)؛ تصویر با og:image از صفحهٔ مقاله (ارجاع منبع حفظ می‌شود؛ بدون تصویر تصادری)؛ AI = دستیار ویرایشی: جستجو از منابع، ترجمه/خلاصهٔ فارسی (مخاطب: زنان)، «چرا مهم است»، نکات کلیدی، دسته‌بندی، رد خودکار بی‌ارزش‌ها؛ انتشار فقط با تأیید مربی + اعتبارسنجی؛ audit + اعلان «📰» |
