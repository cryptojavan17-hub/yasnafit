...
**آخرین به‌روزرسانی:** 2026-09-20 — **Task 27 (T-17 + اصلاح ۲): خط সম্পاد کامل مجله با روال سادهٔ مالک — موتور خودکار با ۶ منبع معتبر پیش‌فرض روز دنیا (PubMed + رسانه‌های علمی/ورزشی؛ ورزش/تغذیه/بدنسازی/به‌ویژه زنان) + تکراری‌زدایی + ترجمه/ویراستاری AI فارسی + صف بررسی مربی + تأیید و انتشار با اعتبارسنجی؛ بدون هیچ تنظیمی («بررسی مطالب جدید» و تمام)؛ ۵۵ چک + npm test + e2e سبز — قبل‌تر: Task ۲۵ مرحلهٔ ۳ از ۴ (مجله home) + Task ۲۶ (ربات تلگرام)**
...
| دیتابیس | `node:sqlite` داخلی (experimental) → `data/yasnafit.db` حالت WAL، `foreign_keys=ON`؛ **۵۵ جدول** (49 موجود + ۴ سایت عمومی در 031 + ۲ خط সম্পاد مجله در 033؛ ۳۴ فقط seed ۶ منبع پیش‌فرض روز دنیا بدون جدول جدید) |
...
| Editorial Admin (پنل مجله) | **IMPLEMENTED** `VERIFIED` (Task ۱۷ — 2026-09-19؛ **Task ۲۷ — 2026-09-20: خط সম্পاد کامل + روال ساده**) | `/coach/magazine` در سایدبار؛ تب‌های مقالات / **در انتظار بررسی** (آمار ۵گانه + «بررسی مطالب جدید» + عملیات بررسی/ویرایش/تأیید و انتشار/رد + صفحهٔ بررسی دوستونه با منبع اصلی و پرچم‌های کیفیت) / **منابع خبری** (پیشرفته؛ ۶ منبع معتبر پیش‌فرض روز دنیا از 034: PubMed تمرینات مقاومتی زنان/تغذیه ورزشی/ورزش و سلامت زنان + Google News علم ورزش/تمرین زنان/علم تغذیه — بدون نیاز به تنظیم) / نتایج شاگردان / دسته‌بندی‌ها / تنظیمات (بازهٔ خودکار)؛ کشف = جستجوی خودکار آن منابع + **ترجمه/ویراستاری AI فارسی** (مخاطب: زنان) با قرارداد «فقط بازنویسی منبع، بدون اختراع»؛ لینک‌های ریدایرکت Google News به آدرس واقعی حل می‌شوند + نام رسانه ثبت می‌شود؛ انتشار فقط با تأیید مربی + اعتبارسنجی (منبع برای generated الزامی، بلاک تکراری)؛ همه تغییرات audit می‌شوند؛ same-origin ۴۰۳ |
