...
| Training Systems | **PARTIAL** | وب: ۵ سیستم؛ فلوی موبایل: ۱۲ سیستم — بنگرید §7 |
...
### WEB — سیستم‌های پیاده‌شده `VERIFIED` (`public/program-builder.js` → systemTypes)
| id | label | type | حرکات |
|---|---|---|---|
| 1 | عادی | normal | ۱ |
| 2 | سوپرست | superset | ۲ |
...
(در صفحه ویرایش حرکت، تگ‌های کمکی DROPSET/SUPERSET/GIANT_SET هم برای عنوان ست وجود دارد.)

### ANDROID — سیستم‌های موردانتظار فلوی موبایل «افزودن نمونه برنامه تمرینی» (مشخصات مالک؛ کد اندرویدی برای راستی‌آزمایی وجود ندارد → `NOT VERIFIED` به‌عنوان کد، `VERIFIED` به‌عنوان مشخصات اعلامی)
۱ حرکتی: **معمولی، رست پاز، دراپ ست، پس خستگی، FST7، ۲۱** • ۲ حرکتی: **سوپر ست، تکرار نیمه** • ۳ حرکتی: **تری ست، ۲۰-۱۰-۵** • ۴ حرکتی: **جايت ست** • ۵ حرکتی: **ماموت ست**

### GAP (شکاف یکپارچه‌سازی — تغییر یک‌طرفه ممنوع)
وب ۵ سیستم دارد؛ فلوی موبایل ۱۲ (۷ مورد در وب غایب‌اند: رست پاز، پس خستگی، FST7، ۲۱، تکرار نیمه، ۲۰-۱۰-۵، ماموت ست). کاتالوگ سیستم‌ها در DB هم وجود ندارد (فقط int + text آزاد در `exercise_systems`).

### NEXT ACTION
۱) مالک لیست کانونی ۱۲گانه را قطعی کند. ۲) کاتالوگ مرجع سیستم‌ها (id/کلید/تعداد حرکت اجباری) در DB + سرویس ساخته شود. ۳) فرانت‌اند وب به همان کاتالوگ وصل شود. ۴) نگاشت idهای قدیمی (۱–۵) حفظ شود تا داده‌های موجود نشکنند.
...
LAST VERIFIED:      2026-08-24 (بعد از بازطراحی UI برنامه‌ساز: ui+terminology+e2e+smoke DOM همه PASS)
...
1. **وب ۵ سیستم تمرینی ↔ موبایل ۱۲ سیستم** (§7) — بزرگ‌ترین شکاف یکپارچه‌سازی.
