...
**به‌روزرسانی همان روز (درخواست مالک، `arena/01a0c4f9-yasnafit`):** سطح عمومی تلگرام ساده شد — دکمهٔ «ربات تلگرام» در هدر همهٔ صفحات عمومی و یک بخش CTA در انتهای لندینگ فقط به `https://t.me/yasnafitbot?start=landing` لینک می‌دهند؛ فرم ثبت آیدی، مودال `tg-dialog` و دو endpoint عمومی `/api/telegram-bot*` حذف شدند (۴۰۱ از گیت مربی، صفر رکورد در `telegram_bot_connections`). `site.telegram_bot_username` همچنان با اعتبارسنجی `^[A-Za-z0-9_]{4,32}$` منبع لینک است و در غیر این صورت به `@yasnafitbot` برمی‌گردد. کارت پروفایل شاگرد = «شناسهٔ تلگرام» (تماس اختیاری) بدون وعدهٔ ارسال. چرخهٔ ربات و پیوند تأییدشده هنوز `NOT IMPLEMENTED` (T-20/KI-023). این تسک روی شاخهٔ جلسه است و سایت عمومی (لندینگ+مجله) از `arena/01a0b993-yasnafit` در همان شاخه merge شد (PR #9).

**Task 31 — همان روز (دستور مالک «صفحهٔ لندینگ جدید رو بزار صفحهٔ اصلیم»):** «/» دوباره لندینگ جدید برای همه است (TD-52)؛ `/home` و `/index.html` ⇒ 301؛ ورود شاگرد روی `/student/login` (پیل «معرفی یسنا فیت» ⇒ `/`)؛ فوتر لندینگ لینک کوچک «ورود مربی» دارد؛ آنالیتیکس بازدید صفحات عمومی را هم می‌شمارد. `npm test` = ۳۱/۳۱، e2e زنده سبز.

**Task 30 — همان روز (ادغام + تصمیم مالک، `arena/01a0c4f9-yasnafit`) — بند «/ = ورود شاگرد» آن در Task 31 برگشت:** پیام «کد اتصال نامعتبر» که مالک از ربات می‌دید مال شاخهٔ merge‌نشدهٔ `arena/01a085de-yasnafit` بود (یکپارچه‌سازی کامل تلگرام + ۴۹ کامیت دیگر: تم روشن، بازبینی ارزیابی، autosave، backup/restore، موبایل، آنالیتیکس، ورود مربی تک‌مرحله‌ای، `/` = صفحهٔ ورود شاگرد) که روی کامپیوتر ویندوز مالک اجرا می‌شود. با انتخاب مالک آن شاخه **در همین شاخه merge شد** (TD-51)؛ سرویس مینیمال Task 29 حذف شد و پیام مهمان (خوش‌آمد محترمانه + راهنمای شروع) داخل `src/telegram-service.js` است. **«/» = صفحهٔ ورود/ثبت‌نام شاگرد** (با پیل «ورود مربی» و «معرفی یسنا فیت») و **لندینگ روی `/home`** (TD-50، TD-21 منسوخ). ورود مربی حالا **بدون کد ۶ رقمی** و ایمیل قفل‌شده `mehdi.javan.64@gmail.com`. `npm test` = ۳۱/۳۱. وضعیت: کد `IMPLEMENTED/VERIFIED (fixture)`؛ پذیرش زنده `UNKNOWN — needs verification` (مالک).

**Task 29 — همان روز (پرامپت دوم مالک، `arena/01a0c4f9-yasnafit`) — سرویس مینیمال آن در Task 30 حذف شد:** ربات `@yasnafitbot` حالا سمت سرور پاسخ‌گو است: `src/telegram-bot-service.js` (بدون وابستگی) به `/start` و `/start landing` پیام خوش‌آمد فارسی مالک + لینک `https://yasnafit.ir/student/register` (لینک + دکمهٔ inline) و به `/help`/پیام‌های دیگر راهنمای کوتاه می‌دهد. دریافت update: webhook `POST /api/telegram/webhook` (پروداکشن؛ محافظت با `X-Telegram-Bot-Api-Secret-Token`) یا long polling (توسعه). توکن فقط از `TELEGRAM_BOT_TOKEN` (هرگز لاگ نمی‌شود). **هیچ ذخیره‌ای نیست** (نه chat_id، نه username، نه اتصال حساب) — طبق دامنهٔ صریح مالک. وضعیت: کد `IMPLEMENTED` و با Bot API جعلی `VERIFIED` (۱۹ گروه)؛ پذیرش با ربات زنده `UNKNOWN — needs verification` تا مالک متغیرها را روی Railway ست کند (`DEPLOYMENT.md` §۱۰).
