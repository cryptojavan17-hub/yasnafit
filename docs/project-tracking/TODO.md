...
### T-2X | یادآور پایان برنامه (PROGRAM_ENDING_REMINDER) + emitهای ارزیابی تلگرام
- **Priority:** P1 • **Status:** OPEN (2026-09-10)
- **Reason:** یکپارچه‌سازی تلگرام (Task 15) تحویل شد؛ این دو قطعه عمداً در دامنه نماندند — شِدول «چند روز قبل از پایان» نیازمند تصمیم مالک است و نقطه‌های emit ارزیابی (approved/rejected/changes) در `server.js` ~۲۲۷۰ شناسایی شده‌اند.
