...
engagementService.notify(db,{audienceType:'coach',studentId,type:'student_message',title:'💬 پیام جدید از شاگرد',body:`👤 ${smStudent?smStudent.full_name:'شاگرد'} (پرونده ${smStudent?smStudent.case_number:'—'})\n📱 ${smStudent?smStudent.mobile:'—'}\n\n${message.body}`,entityType:'conversation'});auditService.record(db,{actorType:'student',actorId:studentId,action:'message.sent',entityType:'conversation',metadata:{sender_type:'student'}});return send(res,201,{message});}catch(error){return sendCaughtError(res,error);}}
