...
| `TELEGRAM_BOT_TOKEN` | خاموش | توکن ربات `@yasnafitbot` از BotFather. تا ست نشود (یا در پنل ثبت نشود) ربات کاملاً خاموش است. **هرگز در git/چت/لاگ نیاید.** جایگزین: پنل مربی → سیستم → تنظیمات تلگرام (متغیر محیطی اولویت دارد) — §۱۰ |
| `TELEGRAM_BOT_USERNAME` | — | یوزرنیم ربات بدون `@` (برای ساخت لینک اتصال و دکمهٔ لندینگ) |
| `TELEGRAM_WEBHOOK_SECRET` | — | رمز تصادفی قوی (`openssl rand -hex 32`)؛ سرور فقط درخواستی را روی `/api/telegram/webhook` می‌پذیرد که هدر `X-Telegram-Bot-Api-Secret-Token` آن با این رمز برابر باشد |
| `YASNAFIT_PUBLIC_URL` | — (ربات: `https://yasnafit.ir`) | آدرس عمومی سایت برای دکمه‌های داخل پیام‌های تلگرام؛ پیام خوش‌آمد مهمان در نبود آن از `https://yasnafit.ir` استفاده می‌کند |
| `TELEGRAM_POLLING` | خاموش | `1` فقط برای توسعهٔ محلی بدون HTTPS: دریافت پیام‌ها با long polling به‌جای webhook |
...
```bash
# یک‌بار: از روی خودِ سرور به /coach/setup بروید و ایمیل + رمز مربی را بسازید.
# ورود مربی تک‌مرحله‌ای است: ایمیل + رمز (مرحلهٔ کد ۶ رقمی حذف شده است).
```
...
3. **متغیر را پاک کنید** و دوباره redeploy کنید. ورود مربی تک‌مرحله‌ای است (ایمیل + رمز) و نیازی به Google Authenticator نیست.
...
### ۹.۸ تأیید دو مرحله‌ای مربی حذف شده است (۲۰۲۶-۰۹-۱۱)

مرحلهٔ کد ۶ رقمی Google Authenticator از جریان ورود مربی **حذف شده است**؛ ورود تک‌مرحله‌ای است: ایمیل + رمز ⇒ نشست ۱۲ ساعته.
صفحهٔ `/coach/2fa`، endpointهای `challenge/verify`، متغیرهای `YASNAFIT_ALLOW_2FA_SKIP`/`YASNAFIT_REVEAL_AUTHENTICATOR_KEY` و اسکریپت `scripts/provision-coach-totp.js` دیگر وجود ندارند. اگر ورودی «Yasnafit» را از قبل در Google Authenticator گوشی دارید، می‌توانید آن را حذف کنید. زیرساخت TOTP در دیتابیس (ستون‌های `totp_*` و جدول چالش‌ها) دست‌نخورده مانده تا برگشت به عقب ساده بماند، ولی هیچ مسیر فعلی از آن استفاده نمی‌کند.
...
## ۱۰. ربات تلگرام `@yasnafitbot` (از ۲۰۲۶-۰۹-۰۹؛ پیام مهمان از ۲۰۲۶-۰۹-۲۱)

کد: `src/telegram-service.js` (بدون وابستگی npm؛ `fetch` داخلی Node ≥ 22) + `src/notification-service.js`. روت دریافت: `server.js` → `POST /api/telegram/webhook`. پنل: مربی → سیستم → **تنظیمات تلگرام** (`/settings/telegram`).

**چه می‌کند:**
* **مهمان** (کسی که با دکمهٔ لندینگ `https://t.me/yasnafitbot?start=landing` یا با `/start` خالی وارد می‌شود): پیام خوش‌آمد محترمانه + «راهنمای شروع» (ثبت‌نام → فرم ارزیابی → اتصال حساب از «پروفایل من») + دو دکمهٔ «ثبت‌نام در سایت» / «ورود به پنل شخصی». `/help` و هر پیام دیگر مهمان ⇒ راهنمای کوتاه. **برای مهمان هیچ چیزی ذخیره نمی‌شود.**
* **شاگرد**: از پنل شخصی → «پروفایل من» → «اتصال تلگرام» با یک کلیک (کد یک‌بارمصرف ۱۵ دقیقه‌ای، `/start <کد>`) وصل می‌شود؛ سپس اعلان‌های برنامه/رژیم/مکمل/پیام مربی/تصمیم ارزیابی به تلگرامش آینه می‌شود؛ منوی `/program /nutrition /notifications /settings /unlink`.
* **مربی**: از «تنظیمات تلگرام» → «اتصال تلگرام مربی» (تا ۳ حساب)؛ اعلان «ارزیابی جدید»، پیام شاگرد، آمار سامانه و بازدید سایت؛ دکمهٔ «ارسال پیام آزمایشی».

### ۱۰.۱ دو حالت دریافت پیام

| حالت | کی | چه می‌کند |
|---|---|---|
| **webhook** (پروداکشن — Railway / سرور با TLS) | پیش‌فرض وقتی `TELEGRAM_POLLING` ست نیست | تلگرام هر update را به `https://<دامنه>/api/telegram/webhook` POST می‌کند. سرور فقط درخواستی را می‌پذیرد که هدر `X-Telegram-Bot-Api-Secret-Token` با `TELEGRAM_WEBHOOK_SECRET` (یا رمز ثبت‌شده در پنل) برابر باشد؛ بدون رمز/هدر ⇒ `401`؛ ربات پیکربندی‌نشده ⇒ `503` |
| **polling** (توسعه / لوکال) | `TELEGRAM_POLLING=1` یا تیک «polling» در پنل | حلقهٔ `getUpdates`؛ در شروع `deleteWebhook` می‌زند. ⚠️ اگر هم‌زمان پروداکشن webhook داشته باشد، polling لوکال آن را حذف می‌کند ⇒ **توکن ربات اصلی را فقط در یک محیط فعال نگه دارید**؛ برای تست لوکال یک ربات آزمایشی دوم از BotFather بسازید |

### ۱۰.۲ راه‌اندازی روی Railway

دو راه (متغیر محیطی اولویت دارد):

1. **Variables:** `TELEGRAM_BOT_TOKEN`، `TELEGRAM_BOT_USERNAME=yasnafitbot`، `TELEGRAM_WEBHOOK_SECRET` (رشتهٔ تصادفی قوی)، `YASNAFIT_PUBLIC_URL=https://yasnafit.ir` (بدون اسلش انتهایی) ⇒ redeploy.
2. **یا از پنل:** مربی → سیستم → تنظیمات تلگرام → توکن/یوزرنیم/رمز وب‌هوک/آدرس عمومی را ذخیره کنید (در جدول `settings` روی Volume می‌ماند).

در لاگ باید ببینید: `[Telegram] فعال است (@yasnafitbot) — منبع: … | webhook: secret ست شده | public URL: https://yasnafit.ir`. بدون توکن فقط: `[Telegram] غیرفعال — برای فعال‌سازی: پنل مربی ← سیستم ← تنظیمات تلگرام …`.

### ۱۰.۳ ثبت webhook — در BotFather چیزی ثبت نمی‌شود

BotFather فقط توکن، نام، توضیحات و فهرست دستورها را نگه می‌دارد؛ webhook با متد `setWebhook` از Bot API ثبت می‌شود. راه‌ها:
* **از پنل:** تنظیمات تلگرام → دکمهٔ ثبت وب‌هوک (`POST /api/coach/telegram/webhook` با آدرس `https://yasnafit.ir/api/telegram/webhook`) — رمز وب‌هوک همان لحظه به تلگرام داده می‌شود.
* **دستی (توکن فقط در همان شل):**
```bash
curl -s -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/setWebhook" \
  -d "url=https://yasnafit.ir/api/telegram/webhook" -d "secret_token=$TELEGRAM_WEBHOOK_SECRET" -d 'allowed_updates=["message","callback_query"]'
curl -s "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/getWebhookInfo"   # url + pending_update_count + last_error_message
```
فهرست دستورهای منو را خودِ سرور در استارت با `setMyCommands` ثبت می‌کند (`registerCommands`). اختیاری در BotFather: `/setdescription`، `/setabouttext`، `/setuserpic`.

### ۱۰.۴ راستی‌آزمایی پس از deploy

1. `getWebhookInfo` (بالا) → `url` = `https://yasnafit.ir/api/telegram/webhook` و `last_error_message` خالی.
2. `curl -i -X POST https://yasnafit.ir/api/telegram/webhook` → `401` (بدون هدر رمز).
3. در تلگرام `https://t.me/yasnafitbot?start=landing` → START → **پیام خوش‌آمد محترمانه + راهنمای شروع** با دو دکمه؛ دوباره `/start landing` ⇒ همان پیام؛ `/help` ⇒ راهنمای کوتاه؛ یک متن دلخواه ⇒ راهنمای کوتاه (نه «کد اتصال نامعتبر»).
4. با یک شاگرد آزمایشی: پنل شخصی → پروفایل من → «اتصال تلگرام» → در تلگرام «✅ حساب شما با موفقیت به یسنا فیت متصل شد».
5. پنل مربی → تنظیمات تلگرام → «ارسال پیام آزمایشی».

### ۱۰.۵ عیب‌یابی

| نشانه | علت / راه‌حل |
|---|---|
| ربات به مهمان می‌گوید «⚠️ کد اتصال نامعتبر است» با `/start landing` | نسخهٔ قدیمی کد (قبل از ۲۰۲۶-۰۹-۲۱) در حال اجراست — سرور را به‌روز و ری‌استارت کنید |
| `getWebhookInfo` → `last_error_message: Wrong response from the webhook: 401` | رمز وب‌هوک سرور با رمزی که هنگام `setWebhook` داده شده فرق دارد؛ webhook را با رمز فعلی دوباره ثبت کنید |
| `last_error_message` دربارهٔ SSL/certificate | تلگرام فقط https معتبر روی پورت‌های 443/80/88/8443 را می‌پذیرد |
| لاگ: `Conflict: terminated by other getUpdates request` (409) | دو پروسه با یک توکن (مثلاً لوکال با polling + سرور). یکی را ببندید یا ربات آزمایشی جدا بسازید |
| ربات جواب نمی‌دهد، `info` سالم | لاگ سرور را برای `[Telegram]` بگردید؛ `pending_update_count` بالا یعنی تلگرام به سرور نمی‌رسد (health را ببینید) |
